# 🐶 Configurar Supabase — Guía paso a paso (app Adopta + Storage)
...
## Paso 3B — Crear el bucket de fotos (Storage)
Las fotos que carguen los usuarios viven en **Storage** (el "disco" de Supabase), no en la tabla. En la tabla solo guardamos la **URL** de cada foto.

En el mismo **SQL Editor** → **+ New query**, pega esto y dale **Run**:

```sql
-- Bucket público para las fotos cargadas
insert into storage.buckets (id, name, public)
values ('fotos', 'fotos', true)
on conflict (id) do nothing;

-- Policies de demo (cualquiera puede ver y cargar; igual de abiertas que la tabla)
...
Esto crea el bucket **`fotos`** y lo marca **público** (para que la galería pueda mostrar las imágenes), y abre `storage.objects` para **ver** y **cargar**. Sin la policy de `insert`, la carga falla con un error de RLS.

> También puedes crearlo desde el panel: **Storage → New bucket → `fotos` → Public**. El SQL es más rápido.
...
- Con el botón **+** creas un perrito nuevo; prueba el botón **"Cargar foto"** y elige una imagen de tu equipo: se carga a Storage y aparece la **vista previa**. Al guardar, el perrito sale en la galería **con tu foto**.
